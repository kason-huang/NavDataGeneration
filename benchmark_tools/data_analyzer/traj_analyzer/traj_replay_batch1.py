...
def gen_batch_size():
    # 计算当前所在卡的GPU容量
    # batch size等于剩余gpu容量除以2GB
    import torch
    gpu_capacity = int(torch.cuda.get_device_properties(0).total_memory / (1024 ** 3)) # GB
...
def main():
    ...
    parser.add_argument("--traj_path", type=str,
                        default="/root/NavDataGeneration/data/traj_datasets/objectnav/hm3d_v1_hd_l3mvn/train/content/")
    ...
    parser.add_argument("--save_video", type=str,
                        default='False', help="whether to save video or not")
    ...
    parser.add_argument("--save_metric", type=str,
                        default='True', help="whether to save metric or not")
    ...
    cfg = habitat.get_config(
        f"/root/NavDataGeneration/NavTrajSampleGeneration/L3MVN/envs/habitat/configs/tasks/objectnav_{args.scene_type}.yaml")
    cfg.defrost()
